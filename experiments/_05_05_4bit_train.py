...
# I/O
# changes regularly
wandb_run_name = "4bit_quantization" + time.strftime("_%m%d_%H:%M:%S")
max_duration = 60*60  # maximum training duration in seconds (default: 1 minute)
wandb_notes = """
run with 4bit quantization.
"""
batch_size = (
    2**10
...
# quantization
use_int8 = True  # Use bitsandbytes int8 quantization
# -----------------------------------------------------------------------------
config_keys = [
    k
...
if init_from == "scratch":
    ...
    model_args["use_int8"] = use_int8  # Pass the quantization flag
    ...
    # create the model
    model_args["use_int8"] = (
        use_int8  # Also pass flag when resuming (though state_dict should handle layers)
    )
    gptconf = GPTConfig(**model_args)
    model = GPT(gptconf)
    ...
    # Manually set use_int8 based on train script config, as from_pretrained doesn't know about it
    model.config.use_int8 = use_int8
    model_args["use_int8"] = use_int8
...
if compile:
    if use_int8:
        print("bitsandbytes int8 quantization enabled, disabling torch.compile.")
        compile = False  # Override compile setting if using int8
    else:
...
def estimate_loss():
    ...
    for split in ["train", "val"]:
        ...
        print(
            f"{split} throughput: {tokens_per_sec / 1e6:.2f} M tokens/s in {elapsed:.2f}s"
        )
...
if wandb_log and master_process:
    ...
    {wandb_notes}
    """
    wandb.init(project=wandb_project, name=wandb_run_name, config=config, notes=notes)
...
while True:
    ...
    if iter_num % eval_interval == 0 and master_process:
        ...
        if device_type == "cuda":
            ...
            memory_stats.update({
                ...
                "gpu/active_pct": (torch.cuda.memory_allocated() / 1e9)
                / total_gpu_memory
                * 100,
                "gpu/reserved_pct": (torch.cuda.memory_reserved() / 1e9)
                / total_gpu_memory
                * 100,
            })
        # CPU memory tracking
        ...
        if losses["val"] < best_val_loss or always_save_checkpoint:
            ...
            if iter_num > 0:
                # Note: Saving checkpoints with bnb quantized layers might require
                # specific handling if the default state_dict() doesn't capture everything.
                # Check bnb documentation if resuming from these checkpoints fails.
                checkpoint = {
                ...
                torch.save(
                    checkpoint, os.path.join(out_dir, f"{wandb_run_name}_ckpt.pt")
                )
    ...
    if iter_num % log_interval == 0 and master_process:
        ...
        if local_iter_num >= 5:  # let the training loop settle a bit
            ...
            print(
                f"iter {iter_num}: loss {lossf:.4f}, time {dt * 1000:.2f}ms, mfu {running_mfu * 100:.2f}%, throughput {throughput / 1e6:.2f}M tokens/s"
            )
    iter_num += 1
    local_iter_num += 1
...
if master_process and wandb_log:
    ...
    wandb.log({"samples": [wandb.Html(f"<pre>{sample}</pre>") for sample in samples]})
