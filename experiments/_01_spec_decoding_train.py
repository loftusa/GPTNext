...
from _01_spec_decoding_model import GPTConfig, GPT # Changed import
...
# changes regularly
DEBUG = False
wandb_run_name = 'spec_dec' + time.strftime("_%m%d_%H:%M:%S") + "_DEBUG" if DEBUG else "" # Changed run name prefix
max_duration = 60*60*3  # maximum training duration in seconds
max_duration = 30 if DEBUG else max_duration
wandb_notes = """
Speculative decoding training run.
"""
if DEBUG: 
    wandb_notes += "\n#########DEBUG RUN#########"
...
max_iters = 25000 # 600000 # total number of training iterations. Set to a large number so that we're bottlenecked by the max duration.
weight_decay = 1e-1
beta1 = 0.9
beta2 = 0.95
...
while True:
    ...
    if iter_num % eval_interval == 0 and master_process:
        ...
        if wandb_log:
            wandb.log({
                "iter": iter_num,
                # "train/loss": losses['train'], # Logged in estimate_loss
                # "val/loss": losses['val'],   # Logged in estimate_loss
                # "train/perplexity": train_ppl, # Logged in estimate_loss
                # "val/perplexity": val_ppl,   # Logged in estimate_loss
                "lr": lr,
