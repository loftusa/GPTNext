...
from _12_muon_model import GPTConfig, GPT
...
muon_optimizer, adamw_optimizer = model.configure_optimizers(
    weight_decay, learning_rate, (beta1, beta2), device_type, ddp_rank if ddp else 0, ddp_world_size
)
if init_from == "resume":
    # Load both optimizer states
    if 'muon_optimizer' in checkpoint:
        muon_optimizer.load_state_dict(checkpoint['muon_optimizer'])
        print("Loaded Muon optimizer state.")
    else:
        print("Warning: Muon optimizer state not found in checkpoint.")
    if 'adamw_optimizer' in checkpoint:
        adamw_optimizer.load_state_dict(checkpoint['adamw_optimizer'])
        print("Loaded AdamW optimizer state.")
    else:
        # Older checkpoints might only have one optimizer key
        adamw_optimizer.load_state_dict(checkpoint['optimizer'])
        print("Loaded optimizer state into AdamW (assuming older checkpoint format).")
    ...
    with torch.no_grad():
        with ctx:
            for k in range(num_samples):
                ...
                sample_throughput = (
                    tokens_generated / sample_time if sample_time > 0 else 0
                )
                ...
                print(f"Sample {k + 1}:\n{sample_text}\n")
                print(
                    f"Generated {tokens_generated} tokens in {sample_time:.2f}s ({sample_throughput:.2f} tokens/sec)\n---------------"
                )

    # Calculate overall inference throughput
    inference_time = time.time() - inference_start_time
    inference_throughput = (
        inference_tokens / inference_time if inference_time > 0 else 0
    )
    print(
        f"Overall inference: {inference_tokens} tokens in {inference_time:.2f}s ({inference_throughput:.2f} tokens/sec)"
    )
